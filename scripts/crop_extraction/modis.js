...
for (var i = 0; i < 4; i++) {
  ...
  var mayMax = MODIS_NBAR
    .filterDate(year + '-04-25', year + '-05-15')
    .map(cal_ndvi)
    .max()
    .clip(region);

  var juneMin = MODIS_NBAR
    .filterDate(year + '-06-10', year + '-06-25')
    .map(cal_ndvi)
    .min()
    .clip(region);

  var mask = ee.Image(mayMax).expression(
    'FIRST > 0.55 && SECOND < 0.45 ? 1 : 0', {
